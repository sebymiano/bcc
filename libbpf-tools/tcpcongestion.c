...
const char *argp_program_version = "tcpcongestion 0.1";
const char *argp_program_bug_address =
	"https://github.com/iovisor/bcc/tree/master/libbpf-tools";
const char argp_program_doc[] =
"Trace latency between TCP received pkt and picked up by userspace thread.\n"
"\n"
"USAGE: tcpcongestion [--help] [-T] [-p PID] [-t TID] [-l LPORT] [-r RPORT] [-w] [-v]\n"
"\n"
"EXAMPLES:\n"
"    tcpcongestion             # Trace all TCP packet picked up latency\n"
"    tcpcongestion -T          # summarize with timestamps\n"
"    tcpcongestion -p          # filter for pid\n"
"    tcpcongestion -t          # filter for tid\n"
"    tcpcongestion -l          # filter for local port\n"
"    tcpcongestion -r          # filter for remote port\n"
"    tcpcongestion 1000        # filter for latency higher than 1000us";
...
static int handle_event(void *ctx, void *data, size_t data_sz)
{
	...
	printf("%-16llx %-16s %-*s %-5d %-*s %-5d %-10u %-10u %-10u %-10u %-10u %-10u %-10u\n",
           e->sock_ident, e->comm, column_width, saddr, ntohs(e->sport), column_width, daddr,
           ntohs(e->dport), e->snd_nxt, e->snd_una, e->snd_cwnd, e->ssthresh, e->snd_wnd, e->srtt, e->rcv_wnd);
...
int main(int argc, char **argv)
{
	...
	if (env.timestamp)
		printf("%-8s ", "TIME(s)");
	printf("%-16s %-16s %-*s %-5s %-*s %-5s %-10s %-10s %-10s %-10s %-10s %-10s %-10s\n",
           "SKADDR", "COMM", column_width, "LADDR", "LPORT", column_width, "RADDR", "RPORT",
           "SND_NXT", "SND_UNA", "SND_CWND", "SSTHRESH", "SND_WND", "SRTT", "RCV_WND");
...
cleanup:
	...
	tcpcongestion_bpf__destroy(obj);
