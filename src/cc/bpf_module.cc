...
BPFModule::BPFModule(unsigned flags, TableStorage *ts, const std::string &other_id)
    : flags_(flags),
...
int BPFModule::load_cfile(const string &file, bool in_memory, const char *cflags[], int ncflags) {
  ...
  if (clang_loader_->parse(&mod_, *ts_, file, in_memory, cflags, ncflags, id_, other_id_))
    return -1;
  return 0;
}
...
int BPFModule::load_b(const string &filename, const string &proto_filename) {
  ...
  if (int rc = b_loader_->parse(&*mod_, filename, proto_filename, *ts_, id_, other_id_))
    return rc;
  if (int rc = annotate())
    return rc;
  if (int rc = finalize())
